...
def run_collection_job(
    ...
    if not force:
        ...
        if not acquired:
            logger.info("scheduler", "run_skipped_lock_unavailable", source="scheduler")
            return False

    start_time = time.perf_counter()
    logger.info("scheduler", "collection_job_started", source="scheduler")
    try:
        ...
        store.finish_daily_run(issue_date, metrics=issue.metrics)
        logger.info("scheduler", "collection_job_finished", source="scheduler", duration=duration)
        return True
    except Exception as exc:
        duration = time.perf_counter() - start_time
        logger.error("scheduler", "collection_job_failed", source="scheduler", duration=duration, error=str(exc))
...
class DailyScheduler:
    ...
    def run_once(self, force: bool = False) -> bool:
        ...
        if not should_run:
            logger.debug("scheduler", "scheduler_tick_skip", source="scheduler", error=reason)
            return False
    ...
    def run_loop(self) -> None:
        ...
        logger.info("scheduler", "scheduler_started", source="scheduler", error=f"time={time_str}, tz={tz_name}")
        while not self._stop_event.is_set():
            try:
                self.run_once()
            except Exception as exc:
                logger.error("scheduler", "scheduler_loop_error", source="scheduler", error=str(exc))
