...
def is_candidate_compatible_with_cluster(
    ...
    # the structured launch context check when an unrelated pricing or release candidate arrives.
    is_pricing_release_interaction = (
        ("pricing" in candidate_actions and "release" in cluster_anchor.actions)
        or ("release" in candidate_actions and "pricing" in cluster_anchor.actions)
    )
    if is_pricing_release_interaction:
