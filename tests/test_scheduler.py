...
def test_should_run_daily_collection(tmp_path):
    ...
    should, date_str, reason = should_run_daily_collection(dt_early, "07:00", store)
    assert should is False
    assert reason == "before_scheduled_time"

    # After scheduled time, not run yet
    dt_late = datetime(2026, 9, 17, 8, 0, tzinfo=tz)
    should, date_str, reason = should_run_daily_collection(dt_late, "07:00", store)
    ...
    # Forced run
    should, date_str, reason = should_run_daily_collection(dt_late, "07:00", store, force=True)
    assert should is True
